...
namespace iQuest.BooksAndNews.Application
{
    public class PrintingUseCase
    {
        ...
        private string SelectTheBookLovers(List<BookLover> bookLovers)
        {
            foreach (BookLover bookLover in bookLovers)
            {
                string name = bookLover.name;
                
            }
            return name;
        }

        private string SelectTheNewsHunters(List<NewsHunter> newsHunters)
        {
            foreach (NewsHunter newsHunter in newsHunters)
            {
                name = newsHunter.name;
